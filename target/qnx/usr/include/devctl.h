 ...
 * Both devctl() and ioctl() encode the direction in the command using
 ...
 *      MEDIA                   0xE00 - 0xEff
 *      CAM - SIM               0xF00 - 0xFff
 *      Filesystem Event Mgr    0x1200-0x12ff
 *
 * _DCMD_MISC is the recommended class for non-QNX devctl()s.
 *
 */
...
// posix_devctl() is a FACE standard function, which looks and behaves exactly
// like devctl().
extern int posix_devctl(int __fd, int __dcmd, void *__dev_data_ptr,
                        size_t __nbytes, int *__dev_info_ptr)
    __ALIASATTR("devctl");
#endif
