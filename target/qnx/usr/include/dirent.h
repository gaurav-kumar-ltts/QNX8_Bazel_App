/*
 * $QNXLicenseC:
 * Copyright 2013-2020, QNX Software Systems. All Rights Reserved.
 *
...
extern int readdir_r(DIR *__dirp, struct dirent *__entry, struct dirent **__result) __ALIAS64("readdir64_r");
#endif

#ifdef __EXT_LF64SRC
extern struct dirent64 *readdir64(DIR *__dirp) __ALIASOFF("readdir");
extern int readdir64_r(DIR *__dirp, struct dirent64 *__entry, struct dirent64 **__result) __ALIASOFF("readdir_r");
...
int _readdir_r(DIR *__dirp, struct dirent *__entry, struct dirent **__result, unsigned bufsize) __ALIAS64("_readdir64_r");
#ifdef __EXT_LF64SRC
#define __EXT_QNX__READDIR64_R 1
extern int _readdir64_r(DIR *__dirp, struct dirent64 *__entry, struct dirent64 **__result, unsigned bufsize) __ALIASOFF("_readdir_r");
#if defined(__EXT_LF64ALIAS) && !defined(__ALIAS_ATTRIBUTE)
#error ALIAS not configured for compiler: readdir
#endif
#endif
...
/**
 * Determine the address of the next dirent structure in an array, as populated
 * by readdir() et al.
 * @param   __dent  A dirent structure
 * return   A pointer to the next dirent structure in an array
 */
static inline struct dirent *
dirent_next(struct dirent * const __dent)
{
    return (struct dirent *)(void *)((_Uintptrt)__dent + __dent->d_reclen);
}
