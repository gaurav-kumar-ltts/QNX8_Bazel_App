...
/*
 * Adding the U suffix to UINT8_MAX and UINT16_MAX would not be compliant with
 * the C Standard as it requires that the "expression shall have the same type
 * as would an expression that is an object of the corresponding type converted
 * according to the integer promotions" (C11/18 clause 7.20.2 paragraph 2). The
 * small integer types are promoted to type int, whereas a U suffixed integer
 * literal has the type unsigned int.
 */
