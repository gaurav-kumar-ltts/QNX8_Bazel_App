...
* @c capture_get_frame().
*
* The thread which is blocked waiting for capture_get_frame() can be unblocked
* by setting CAPTURE_ENABLE to 0, followed by capture_update(), in a different thread.
* When this is done, or if capture_get_frame() is called with capture already disabled,
* the call will fail.
*
...
/**
* @brief When written, try to reinitialize capture hardware.
*        A reinitialization might be needed after some specific device state
*        changes. In general, capture drivers handle the reinitialization on
*        their own, but in some corner cases, such as coordination between
*        multiple capture devices, the user may use this property directly.
*
...
*/
/**
* @brief  Source format; the format of the stream of pixels from the video
*         source
*
*         Some valid values for this property are defined by
...
*
*         If @c CAPTURE_PROPERTY_SRC_FORMAT is different from
*         @c CAPTURE_PROPERTY_DST_FORMAT, the capture library will convert the
*         captured data from @c CAPTURE_PROPERTY_SRC_FORMAT to
*         @c CAPTURE_PROPERTY_DST_FORMAT, if the conversion is supported by
...
*
*         The valid values for this property are defined by Screen pixel format
*         types in @c screen.h.
*
*         Read/Write @c uint32_t
*
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/release-4.0.2/lib/capture/public/vcapture/capture.h $ $Rev: 939870 $")
#endif
