...
#define CAPTURE_PROPERTY_ADV_WAIT4LOCK_MS	CAPTURE_PROPERTY( 'A', 'D', 'W', 'L' )
	/* R/W		uint32_t		Sets/reads the maximum time (in milliseconds) the driver will wait for input signal
	 *					to be locked when capture is being enabled. The default value is 2000ms. */

#define CAPTURE_PROPERTY_ADV_OUTPUT_IFACE	CAPTURE_PROPERTY( 'A', 'D', 'O', 'I' )
	/* R/W		uint32_t		Sets/reads the output interface. For ADV748x only.
	 *					Use -1 for default (CSI_TXA for CP core; CSI_TXB for SDP core.) */

enum adv748x_output_iface {
	CAPTURE_ADV_OUT_CSI_TXA = 0,
	CAPTURE_ADV_OUT_CSI_TXB,
	CAPTURE_ADV_OUT_8BIT_DIGITAL,
};
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/release-4.0.2/hardware/capture/public/public/vcapture/capture-adv-ext.h $ $Rev: 976553 $")
#endif
