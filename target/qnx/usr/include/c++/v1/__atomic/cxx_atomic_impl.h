...
#if defined(__QNX__)
  static _LIBCPP_CONSTEXPR std::size_t __alignment = 1ULL
                                 << (sizeof(std::size_t) * numeric_limits<unsigned char>::digits
                                                         - __libcpp_clz((sizeof(_Tp) > alignof(_Tp)
                                                                         ? sizeof(_Tp)
                                                                         : alignof(_Tp)) - 1));
  alignas(__alignment > 16 ? 16 : __alignment) _Tp __a_value;
#else
  _Tp __a_value;
#endif
};
