...
oses=nto win64 linux unix $(EXTRA_OS_NAMES)
...
metasrcs=$(foreach dir, $1, $(addprefix $(dir)/*., s S c ${cxx_exts} $(EXTRA_SUFFIXES)))
ifndef SRCS
...
ifeq ($(COMPILER_DRIVER),qcc)
LDPREF_QCC = $(LD$(LINKER_TYPE)$(EX_CLASS)$(select_compiler)) $(linktype_$(LTYPE))
LDPREF_QCC_CMD = $(firstword $(LDPREF_QCC))
LDPREF_GPP_CMD = $(subst qcc,q++,$(LDPREF_QCC_CMD))
LDPREF = $(subst $(LDPREF_QCC_CMD),$(LDPREF_GPP_CMD),$(LDPREF_QCC))
endif
...
# Override target building behaviour for Coverity as we don't need to build the final binary nor link
# to any libraries
ifneq ($(OVERRIDE_COVERITY_TOOLCHAIN),)
define TARGET_BUILD
	$(RM_HOST) $@
	touch $@
endef
...
define TARGET_HINSTALL
	$(PRE_HINSTALL)
	-$(foreach dir, $(PUBLIC_INCVPATH), $(foreach file, $(shell $(FL_HOST) $(dir)), $(CP_HOST) $(file) $(INSTALL_ROOT_HDR)/$(patsubst $(dir)/%,%,$(file))$(NEWLINE)))
	$(POST_HINSTALL)
endef
...
ifndef CLEAN
CLEAN=*.o *.err *.map mapfile *.sym *.i *.dep $(DEFFILE) *.gcno *.gcda
endif

define TARGET_CLEAN
	$(PRE_CLEAN)
	$(RM_HOST) -r $(ICLEAN) $(CLEAN) $(EXTRA_CLEAN) $(EXTRA_ICLEAN)
	$(POST_CLEAN)
endef
...
ifeq ($(COMPILER_TYPE),gcc)
ifeq ($(COMPILER_DRIVER),qcc)
CCDEPS=-Wc,-MT,$@ -Wc,-MMD -Wc,-MP -Wc,-MF,$*.dep
else
CCDEPS=-MT $@ -MMD -MP -MF $*.dep
endif
endif
...
COMPILE_c_o=$(CCWRAP) $(CCPREF) $(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $(CPPOPTS) $(CCDEPS) $< $(CCPOST)
COMPILE_cxx_o=$(CCWRAP) $(CCPREF) $(CXXFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CXXOPTS) $(CPPOPTS) $(CCDEPS) $< $(CCPOST)
COMPILE_nda=$(CP_HOST) $< $@

COMPILE_S_i=$(ASWRAP) $(ASPREF) -E $(ASFLAGS) $(FLAGS) $(ASVFLAGS) $(ASOPTS) $< >$@ $(ASPOST)
COMPILE_c_i=$(CCWRAP) $(CCPREF) -E $(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $(CPPOPTS) $< >$@ $(CCPOST)
COMPILE_cxx_i=$(CCWRAP) $(CCPREF) -E $(CXXFLAGS) $(FLAGS) $(CXXVFLAGS) $(CPPFLAGS) $(CXXOPTS) $(CPPOPTS) $< >$@ $(CCPOST)
...
isr_infix=$(filter-out $(CCFLAGS_ISR_EXCLUDE) $(CCVFLAGS_ISR_EXCLUDE),$(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $(CPPOPTS) $(CCFLAGS_ISR) $(CCVFLAGS_ISR))
