...
# Ignore library dependencies
ifneq ($(QNXBUILD_IGNORE_LIB_DEPS),)
build_line=$(OBJS) $(EXTRA_DEPS)
else
...
-include $(subst .o,.dep,$(filter %.o,$(OBJS)))

# Compiling with coverage enabled and disable optimizations
ifneq ($(OS),win64)
gcov:CPPOPTS += -DTEST_COVERAGE
gcov:CCOPTS+=-O0 -fprofile-arcs -ftest-coverage
ifneq ($(PROFILE_DIR),)
gcov:CCOPTS += -fprofile-dir=$(PROFILE_DIR)
endif
gcov:LDOPTS+=-fprofile-arcs
gcov:install
else
gcov:
	$(warning gcov is not supported on win64)
endif
