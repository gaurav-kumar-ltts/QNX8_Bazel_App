...
#if !defined(__NTO_FORTIFY_LEVEL)
...
#if !defined(__OBJ_BOUNDED_MEMBER_PTR)
	#define __OBJ_BOUNDED_MEMBER_PTR(obj, member)  (&((obj)->member))
#endif

#if !defined(__PAYLOAD_OF)
	/* <STAN_MACRO5> */
	/**
	 * Evaluates to a pointer to the payload which follows the specified header member of an object. This pointer has same type and
	 * value as the expression <tt>(void *)(&(obj)->hdr_member + 1)</tt>, but for the purposes of object size determination
	 * (e.g. @c __builtin_object_size) the compiler associates it with the same object bounds as the @c obj pointer.
	 *
	 * @param[in]  obj         Pointer to the object which contains both header and payload data. The referenced type of this
	 *                         pointer must define the member specified by the @c hdr_member parameter, but need not define any
	 *                         member(s) to represent the payload.
	 * @param[in]  hdr_member  Designator of the member of the specified object that represents the fixed-size header that precedes
	 *                         the payload. This parameter is similar to the second parameter of the @c offsetof macro, but while
	 *                         @c offsetof returns the offset to the start of the specified member of a type, @c __PAYLOAD_OF
	 *                         returns a pointer to the location that immediately follows the specified member of an object. The
	 *                         specified header member may be offset from the start of the referenced type of @c obj, and may be
	 *                         defined within one or more sub-structures and/or sub-unions.
	 *
	 * @return  Pointer to the payload that follows the specified header member.
	 */
	#define __PAYLOAD_OF(obj, hdr_member)  ((void *)(__OBJ_BOUNDED_MEMBER_PTR((obj), hdr_member) + 1))
	/* </STAN_MACRO5> */
#endif
...
The __ALIAS64 and __ALIASOFF macros implement the logic described above for the __EXT_FUNCALIAS64 and __EXT_LF64ALIAS internal
feature test macros. Like the __ALIASATTR macro, they are employed in a similar fashion to GCC's function attributes, but with the
additional restriction that each must immediately follow the parameter list of the relevant function prototype (other function
attributes may then follow).

The __ALIAS64 macro makes a standard function (e.g. pread) into an alias pointing to the specified explicitly-64-bit function
(e.g. "pread64") when __EXT_FUNCALIAS64 is defined. The __ALIAS64ELSE macro has the same effect, but it also specifies an
alternative alias target for the case where __EXT_FUNCALIAS64 isn't defined.

The __ALIASOFF macro makes an explicitly-64-bit function (e.g. pread64) into an alias pointing to the specified standard function
(e.g. "pread") when __EXT_LF64ALIAS is defined. The __ALIASOFFELSE macro has the same effect, but it also specifies an alternative
alias target for the case where __EXT_LF64ALIAS isn't defined.
...
#define __ALIAS64ELSE(__n, __m)	__ALIASATTR(__n)
#else
#define __ALIAS64(__n)
#define __ALIAS64ELSE(__n, __m)	__ALIASATTR(__m)
#endif

#if __LONG_BITS__ == 64
#define __EXT_LF64ALIAS
#define __ALIASOFF(__n)	__ALIASATTR(__n)
#define __ALIASOFFELSE(__n, __m)	__ALIASATTR(__n)
#else
#define __ALIASOFF(__n)
#define __ALIASOFFELSE(__n, __m)	__ALIASATTR(__m)
#endif
