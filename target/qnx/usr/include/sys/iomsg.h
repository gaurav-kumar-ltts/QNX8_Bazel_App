...
enum _io__Uint16types {
	...
	_IO_MMAP,
	_IO_MSG,
	_IO_RSVD_CHECK_MSG,		/* Place holder in jump table */
	_IO_DUP,
	_IO_CLOSE,
...
enum _io_connect_extra_type {
	...
	// Minimum value for extra type guaranteed not to clash with later libc values
	_IO_CONNECT_EXTRA_EXTERNAL_BASE = 64,
};
...
#define _IO_CONNECT_EFLAG_SYSTEM_ABS	0x08	/* System absolute path           */
#define _IO_CONNECT_EFLAG_PATH_NEEDED	0x10	/* Client needs resolved path	  */
...
typedef union _io_link_extra {
	...
	__FLEXARY(char, path);							/* EXTRA_SYMLINK/EXTRA_LINK */
} io_link_extra_t;
...
typedef union {
	...
	// Reply must be provided by using _IO_SET_PATHCONF_VALUE
	//long						o;
} io_pathconf_t;
...
/* <STAN_MACRO5> */
/**
 * Evaluates to a pointer to the variable-length payload that follows the header represented by the @c i member of an IO message
 * object.
 *
 * @param[in]  msg  Pointer to an IO message object of a type that defines an @c i member.
 *
 * @return  Pointer to the variable-length payload that follows the @c i member.
 */
#define _IO_INPUT_PAYLOAD(msg)		(__PAYLOAD_OF((msg), i))

/**
 * Evaluates to a pointer to the variable-length payload that follows the header represented by the @c link_reply member of an IO
 * message object.
 *
 * @param[in]  msg  Pointer to an IO message object of a type that defines a @c link_reply member.
 *
 * @return  Pointer to the variable-length payload that follows the @c link_reply member.
 */
#define _IO_LINK_REPLY_PAYLOAD(msg)	(__PAYLOAD_OF((msg), link_reply))

/**
 * Evaluates to a pointer to the variable-length payload that follows the header represented by the @c o member of an IO message
 * object.
 *
 * @param[in]  msg  Pointer to an IO message object of a type that defines an @c o member.
 *
 * @return  Pointer to the variable-length payload that follows the @c o member.
 */
#define _IO_OUTPUT_PAYLOAD(msg)		(__PAYLOAD_OF((msg), o))
/* </STAN_MACRO5> */
