/*
 * Copyright (c) 2014-2023, BlackBerry Limited.
 *
...
/*
 * Desc:   This call is made to change the mode and ownership of the slave pseudo-terminal device associated
 *         with its master pseudo-terminal counterpart
 * Notes:  This command is used in lib/c/grantpt()
 */
#define DCMD_CHR_GRANTPT __DION(_DCMD_CHR, 42)

/*
 * Desc:   This call is made to change the mode and ownership of the slave pseudo-terminal device associated
 *         with its master pseudo-terminal counterpart. The UID/GID/mode are provided explicitly
 *         rather than chosen by devc-pty.
 */
typedef struct {
	uid_t	uid;
	gid_t	gid;
...
#define DCMD_CHR_GRANTPT_EXPLICIT __DIOT(_DCMD_CHR, 43, grant_pt_explicit_t)
