...
typedef struct _iofunc_mount {
    ...
    uint32_t                        ext_flags;  /* Extended Flags, IOFUNC_PC_EXT_* */
...
#define IOFUNC_MOUNT_FLAGS          0x000000FF  /* Client-controlled bits from io_mount_extra_t */
...
typedef struct _iofunc_ocb {
...
#if !defined(_IOFUNC_OFFSET_BITS) || _IOFUNC_OFFSET_BITS == 64
 #if __OFF_BITS__ == 64
    off_t                           offset;     /* read/write/seek location for file-like devices */
 #else
    off64_t                         offset;     /* read/write/seek location for file-like devices */
 #endif
#elif _IOFUNC_OFFSET_BITS - 0 == 32
 #if __OFF_BITS__ == 32
  #if defined(__LITTLEENDIAN__)
 ...
 #else
  #if defined(__LITTLEENDIAN__)
    int32_t                         offset;    /* read/write/seek location for file-like devices */
    int32_t                         offset_hi;
  #elif defined(__BIGENDIAN__)
    int32_t                         offset_hi;
    int32_t                         offset;    /* read/write/seek location for file-like devices */
...
struct _iofunc_acl_funcs {
	...
	void						(*update)(IOFUNC_ATTR_T *attr);
};
...
extern int iofunc_open_default_cinfo( resmgr_context_t *__ctp, io_open_t *__msg, iofunc_attr_t *__attr, void *__extra, struct _client_info *__cinfo );
...
extern int iofunc_check_msg_default(resmgr_context_t const *__ctp,
                                    resmgr_iomsgs_t const *__msg, bool __combine,
                                    iofunc_ocb_t const *__ocb);
/* If iofunc_lock_default is used, iofunc_close_dup_default and iofunc_unblock_default must be used or called */
