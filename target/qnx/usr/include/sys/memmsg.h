/*
 * $QNXLicenseC:
 * Copyright 2007, 2009, 2021 QNX Software Systems. All Rights Reserved.
 *
...
enum {
	_MEM_MAP_INFO_VIRTUAL,		//virtual memory region info
	_MEM_MAP_INFO_PHYSICAL,		//contigous physical pages having the same attributes
	_MEM_MAP_INFO_PAGETABLE,	//page table entry data
	_MEM_MAP_INFO_OBJECT_PATH,	//path to mapped object
};
...
struct _mem_as_info {
	_Uint16t	type;		// _MEM__AS_INFO
	_Uint16t	subtype;
	_Int32t		pid;
};

typedef union {
	struct _mem_as_info	   i;
	struct _debug_aspace   o;
} mem_as_info_t;


/*
 * Message of _MEM_MAP_INFO
 */

typedef struct _mem_map_info_i {
    _Uint16t type;
    _Uint16t subtype;
    _Uint32t reserved;
    pid64_t pid;
...
/********************************/
/* union mem_map_info_t			*/
/********************************/
typedef union {
	struct _mem_map_info_i   	i;
	struct _procfs_map_info		o;
	struct _procfs_debug_info  	o_dbg;
} mem_map_info_t;


/*
 * Message of  _MEM_READ_MEM
 */
typedef struct  {
	struct _mem_map_info_i   	i;
} mem_read_mem_t;
