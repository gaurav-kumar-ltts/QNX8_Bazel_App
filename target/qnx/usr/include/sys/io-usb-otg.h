...
#define IOUSB_HCD_VERSION			( (uint32_t)0x0002U )
#define IOUSB_DCD_VERSION			( (uint32_t)0x0001U )

/* The following version numbers control the interface 
 * presented to the server by the devu library.  This 
 * includes io_usb_otg_dll_entry_t and referenced data 
 * stuctures. */
#define DEVU_HCD_VERSION			( (uint32_t)0x0001U )
#define DEVU_DCD_VERSION			( (uint32_t)0x0000U )
...
#define _IO_USB_VER_NFUNCS		(( IOUSB_HCD_VERSION << 16 ) | ( (uint32_t)_IO_USB_NFUNCS ) )
#define _IO_USB_VERSION(_self)	(( (_self)->self.hcd->version_nfuncs >> 16 ) & 0xFFFFU)
...
#define _IO_USBDC_VER_NFUNCS		(( IOUSB_DCD_VERSION << 16 ) | ( (uint32_t)_IO_USBDC_NFUNCS ) )
#define _IO_USBDC_VERSION(_self)	(( (_self)->self.dcd->version_nfuncs >> 16 ) & (uint32_t)0xFFFFU)
...
typedef struct _usb_dcd_methods {
	int						nentries;
	int						(*dc_init)					( usb_dcd_t *udcd, uint32_t flags, char *args );
...
#define USB_CD_FLAGS_DEBUG_INIT			0x00000001
#define USB_CD_FLAGS_INIT				0x00000002	/* flag to indicate that controller should be made ACTIVE */
