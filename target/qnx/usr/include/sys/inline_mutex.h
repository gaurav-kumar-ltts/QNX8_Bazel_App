...
static inline int
_mutex_lock(struct _sync_mutex * const mux)
{
	...
	return SyncMutexLock_r((sync_t *)mux);
}

static inline int
_mutex_trylock(struct _sync_mutex * const mux)
{
	const unsigned	owner = __tls()->__owner;
...
static inline int
_mutex_unlock(struct _sync_mutex * const mux)
{
	const unsigned	owner = __tls()->__owner;
	...
	return SyncMutexUnlock_r((sync_t *)mux);
}

static inline int
_mutex_haslock(const struct _sync_mutex * const mux)
{
	return ((mux->__owner & ~_NTO_SYNC_WAITING) == __tls()->__owner);
}

static inline unsigned
_mutex_owner(const struct _sync_mutex * const mux)
{
	unsigned	owner;
...
static inline void
_mutex_init(struct _sync_mutex * const mux)
{
	/*
	 ...
	 */
	const struct _sync_mutex	init_mux = { {0U}, _NTO_SYNC_MUTEX_FREE };
	*mux = init_mux;
}

static inline int
_mutex_isfree(const struct _sync_mutex * const mux)
{
	switch (mux->__owner) {
