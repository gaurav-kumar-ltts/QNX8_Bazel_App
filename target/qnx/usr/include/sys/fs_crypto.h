...
enum _tagFsEncryptTypes {
	...
	FS_CRYPTO_TYPE_XTS,
	FS_CRYPTO_TYPE_CBC,
	FS_CRYPTO_TYPE_CBCV2,
	FS_CRYPTO_TYPE_CBC_128,
	FS_CRYPTO_TYPE_MAX_IMPL         = FS_CRYPTO_TYPE_CBC_128,
...
enum _tagFsEncryptActions {
	...
	FS_CRYPTO_HARD_LOCK_DOMAIN,
	FS_CRYPTO_WHITELIST,
	FS_CRYPTO_REGISTER_DOUBLE_DAR,
	FS_CRYPTO_ENABLE_DAR,
	FS_CRYPTO_DAR_PREPARE_FOR_SWITCH,
	FS_CRYPTO_DAR_UPGRADE_COMPLETE,
	FS_CRYPTO_DAR_COMPLETE_SWITCH,
	FS_CRYPTO_DESTROY_DAR,
	FS_CRYPTO_DAR_OPERATIONAL,
	FS_CRYPTO_MIGRATE_DOMAIN,
	FS_CRYPTO_DISABLE,
...
#define FS_CRYPTO_CMD_STR_LIST					\
	{							\
		...
		"Hard Lock Domain",				\
		"Whitelist",					\
		"Register as double DAR provider",		\
		"Enable Double DAR Encryption",			\
		"Double DAR prepare for OTA switch",		\
		"Double DAR OTA Upgrade complete",		\
		"Double DAR OTA Complete Switch",		\
		"Double DAR Destroy DAR",			\
		"Double DAR Query if operational",		\
		"Migrate Domain",				\
		"Disable Encryption",				\
    }
...
/*	Flags used to control domain hard locking.
*/

/*	Mask for flags controlling when a hard lock will be enforced.
*/
#define FS_CRYPTO_HARD_LOCK_ACTION_MASK		(0x00000003u)

/*	The request should queue the lock of the domain. All filesystem clients that
	have not been whitelisted will be denied access to the keys associated with
	the domain. A queued hard lock can be enforced at a later time by sending
	an additional hard lock request with FS_CRYPTO_HARD_LOCK_ENFORCE used as
	the enforcement flag. If no enforcement flag is specified, this is the
	default behavior.
*/
#define FS_CRYPTO_HARD_LOCK_ACTION_QUEUE	(0x00000000u)

/*	The request should enforce a hard lock of the domain immediately. All
	filesystem clients will be denied access to the keys associated with the
domain.
*/
#define FS_CRYPTO_HARD_LOCK_ACTION_ENFORCE	(0x00000001u)

/*	The request should cancel a queued hard lock. This effectively disables the
	whitelist controlling access to the domain when a hard lock is pending. If a
	hard lock has not been queued, this action shall have no effect. This
	implies that the lock state is not changed by this action. For example, if
	the domain is already locked, this will not unlock the domain since a hard
	lock cannot be queued on a locked domain.
*/
#define FS_CRYPTO_HARD_LOCK_ACTION_CANCEL	(0x00000002u)

/*	Domain whitelisting actions.
*/
#define FS_CRYPTO_WHITELIST_ACTION_SHFT				(0)
#define FS_CRYPTO_WHITELIST_ACTION_MASK				(0x000000FFu)
#define FS_CRYPTO_WHITELIST_ACTION_ACCESS_GRANT		(0x00000001u)
#define FS_CRYPTO_WHITELIST_ACTION_ACCESS_REVOKE	(0x00000002u)
#define FS_CRYPTO_WHITELIST_ACTION_CONFIGURE		(0x00000003u)

/*  Domain whitelist data types.
*/
#define FS_CRYPTO_WHITELIST_TYPE_SHFT               (8)
#define FS_CRYPTO_WHITELIST_TYPE_MASK               (0x0000FF00u)
#define FS_CRYPTO_WHITELIST_TYPE_PID                (0x00000100u)
#define FS_CRYPTO_WHITELIST_TYPE_GID                (0x00000200u)

/*  Domain whitelisting flags.
*/
#define FS_CRYPTO_WHITELIST_FLAG_SHFT               (24)
#define FS_CRYPTO_WHITELIST_FLAG_MASK               (0xFF000000u)
#define FS_CRYPTO_WHITELIST_FLAG_BLACKLIST          (0x01000000u)

/*  Flags used when creating new domain and file keys. Like the key type,
    these flags will be passed down from the domain key to the new file
    keys that get created under the same domain.
*/

/*  Embed a SHA256 HMAC digest into the new key to validate the key data
    before it is used to decrypt or encrypt data.
*/
#define FS_CRYPTO_ENABLE_SHA256_HMAC    (0x0001)


/*  Flags used when reading file keys.
*/
#define FS_CRYPTO_READ_FILE_ONLY    0x0001
#define FS_CRYPTO_READ_DOMAIN_ONLY  0x0002
#define FS_CRYPTO_READ_NO_FILE_DATA 0x0004


/*  Flags when enabling encryption.
*/
#define FS_CRYPTO_ENABLE_DEFAULT 0x0000
#define FS_CRYPTO_ENABLE_NAME    0x0001

/*  Flags when disabling encryption.
*/
#define FS_CRYPTO_DISABLE_DEFAULT 0x0000
#define FS_CRYPTO_DISABLE_NAME    0x0001

/*  Flags for dirent crypto.
 */
#define FS_CRYPTO_DIRENT_DECRYPT 0x0000
#define FS_CRYPTO_DIRENT_ENCRYPT 0x0001
...
/*  Migration of files into an encrypted domain is completed in units of work
	...
	performance.  Zero indicates the file system should make a decision
    ...
    uint32_t version)
{
    ...
    switch (version) {
    case 1:
        size  = (uintptr_t) (&((migrate_status_t *)0)->s_migrate_error);
...
#define FS_CRYPTO_CBC_KEY_SIZE          (256/CHAR_BIT)
#define FS_CRYPTO_CBC_128_KEY_SIZE      (128/CHAR_BIT)
#define FS_CRYPTO_XTS_KEY_SIZE          (512/CHAR_BIT)
...
enum _tagFsEncryptReply {
	...
	FS_CRYPTO_REPLY_BUSY_NAME_ENCRYPT,
...
#define FS_CRYPTO_REPLY_STR_LIST						\
	{									\
		...
		"No support",							\
		"Not a mount point",						\
		"Domain is being used for name encryption",			\
	}
