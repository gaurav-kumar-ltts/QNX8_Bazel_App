...
/**
 * @addtogroup ioreq_flags Flags for Disk I/O requests
 * @{
 */
...
/**
 * Memory Descriptor List element
 *
 * Describes the data buffers associated with an I/O request. Every
 * struct _ioreq will have some number of these to identify the
 * memory for the disk controller to either source/sink.
 *
 * Each of these structures specifies a single contiguous (both
 * physically and virtually) range of memory.
 */
struct _mdl {
	paddr64_t	paddr;    /**< Physical address */
	caddr_t		vaddr;    /**< Virtual address  */
	_Uint32t	length;   /**< Number of contiguous bytes */
#if ( __PTR_BITS__ == 64 )
	_Uint32t	align;
#endif
};
 ...
 */
struct _param {
	baddr_t	lba;			/**< Starting Logical block address              */
	int		length;			/**< Number of blocks                            */
	int		blksize;		/**< Block size used by the length field         */
	int		resid;			/**< Residual xfer length (failed/untried blks)  */
	void	*buflist;		/**< Block/cache buffer list (private to io-blk) */
};
...
/**
 * I/O request from io-blk to the driver layer.
 *
 * This structure allows io-blk to describe an I/O request for a single
 * contiguous run of disk LBA.  Each of these structures is associated
 * with a list of struct _mdl.
 */
struct _ioreq {
	unsigned            flags;                     /**< bitfield of @ref ioreq_flags          */
	int                 status;                    /**< Request status/errno                  */
	int                 priority;                  /**< Client request priority               */
	int                 nmdl;                      /**< Number of elements @c mdl points to   */
	mdl_t               *mdl;                      /**< Pointer to mdl entries                */
	param_t             parm;                      /**< io-blk details                        */
	void                *user;                     /**< Opaque user/private data              */
	int                 (*notify)(ioreq_t *ioreq); /**< Invoked when I/O is complete          */
	ioreq_t             *next;                     /**< Linked-list of related IO             */
	TAILQ_ENTRY(_ioreq) qlink;                     /**< Private use of driver                 */
	sem_t               wait;                      /**< sync iodone wait object               */
	ioreq_t             *prev_done;                /**< Pointer to previously completed ioreq */
};
