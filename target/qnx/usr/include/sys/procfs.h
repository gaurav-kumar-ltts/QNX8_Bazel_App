...
typedef debug_thread_t          procfs_status32;
typedef debug_thread_t          procfs_status64;
...
static inline _Uint32t
procfs_ability_total_size(
    _Sizet num_abilities,
    _Sizet num_ranges)
{
    _Sizet const total  = sizeof(procfs_abilities)
                        + ((num_abilities - 1U) * sizeof(_Uint16t))
                        + (num_ranges * sizeof(procfs_ability_range));
    return (_Uint32t)total;
}
#define PROCFS_ABLE_TOTAL_SIZE(n, r) procfs_ability_total_size(n, r)
