...
*/
#define _SCREEN_VERSION_MINOR  3
...
/** @brief The feature preprocessor definition that indicates the definition of
 *         a connection type that's used to identify a VIRTUAL connection to
 *         the display. For more information, see the #SCREEN_DISPLAY_TYPE_VIRTUAL display type.
 */
#define SCREEN_DISPLAY_TYPE_VIRTUAL SCREEN_DISPLAY_TYPE_VIRTUAL
...
/** @brief The feature preprocessor definition that indicates the definition of
 *         the #SCREEN_PROPERTY_DVADDR_FD property.
 */
#define SCREEN_PROPERTY_DVADDR_FD SCREEN_PROPERTY_DVADDR_FD
...
enum {
	...
	*   are expected to change. If a window is marked as static, Screen bypasses composition when possible. To learn more,
	*   visit the <a href="../../com.qnx.doc.neutrino.fastboot/topic/optimize_app_optimize_windows_screen.html">
	*   Optimize the usage of windows from Screen</a> page.
	*   
	*   When retrieving or setting this property type, ensure that you have
	*   sufficient storage for one integer.
	...
	*   - Change event sent: No
	*   - Applicable to the following Screen API object(s):
	*     - buffer (ready only): will inherit the usage from the parent object (window, stream, pixmap)
	*     - pixmap
	*     - stream
	*     - window
	*/
	...
	*   <a href="screen_8h_1Screen_Scale_Quality_Types.xml">Screen scaling quality types</a>.
	*   When retrieving or setting this property type, ensure that you have
	*   sufficient storage for one integer.
	*   - Configurable: No
	*   - Default privileges: @code rw-rw-r-- @endcode
	...
	*   <a href="screen_8h_1Screen_Scale_Types.xml">Screen scaling mode types</a>.
	*   When retrieving or setting this property type, ensure that you have
	*   sufficient storage for one integer.
	*   - Configurable: No
	*   - Default privileges: @code r--r--r-- @endcode
	...
	*		 For virtual displays, this property can be set based on write privileges for a given context.
	*                For physical displays, this property can never be set.
	*/
	...
	*/
	...
	/** A pointer to a single integer that indicates the file descriptor to 
	*	be used to access a buffer's memory.  Used to provide external memory 
	*   to screen.
	*   When retrieving or setting this property type, ensure that you provide
	*   sufficient storage for one @c void pointer.
	*   - Configurable: No
...
enum {
	...
	*/
	...
	/** A virtual connection type to the display */
	SCREEN_DISPLAY_TYPE_VIRTUAL = 0x7676
};
 ...
 *  @anonenum Screen_Scale_Types Screen scaling mode types
 */
enum {
...
*   @anonenum Screen_Scale_Quality_Types Screen scaling quality types
*/
enum {
...
enum {
	...
	*/
	SCREEN_EVENT_DEVICE __attribute__((deprecated("Deprecated, will be removed in a later release"))) = 16,
...
enum {
	...
	*/
	...
	SCREEN_DONT_FLUSH __attribute__((deprecated("Deprecated, will be removed in a later release"))) = (1 << 2),
	/** Internal use only; do not use. Deprecated, will be removed in a later release */
	SCREEN_POST_RESUME __attribute__((deprecated("Deprecated, will be removed in a later release"))) = (1 << 3),
	/** Internal use only; do not use. */
...
enum {
	...
	*/
	...
	/* Below are flags used to turn on the debug overlay feature.
		Debug overlay impacts final composition.
	*/

	/** Category 1, basic overlay: win id, FPS, stream and associated buffers */
	SCREEN_DEBUG_OVERLAY_CATEGORY_1 = (1 << 8),

	/** Category 2, composition overlay: composition reason, composition module */
	SCREEN_DEBUG_OVERLAY_CATEGORY_2 = (1 << 9),

	/** Category 3, dynamic status overlay: total posts, posts per second, updates per second */
	SCREEN_DEBUG_OVERLAY_CATEGORY_3 = (1 << 10),

	/** Category 4, static stats overlay: window format, stream format, transparency, src size, dst size, viewport size, win id string */
	SCREEN_DEBUG_OVERLAY_CATEGORY_4 = (1 << 11),

	/** Category 5, visual debug overlay: window outline */
	SCREEN_DEBUG_OVERLAY_CATEGORY_5 = (1 << 12),

    /** Mask to enable more than one debug overlay level */
	SCREEN_DEBUG_OVERLAY_MASK = (SCREEN_DEBUG_OVERLAY_CATEGORY_1 | SCREEN_DEBUG_OVERLAY_CATEGORY_2 |
		SCREEN_DEBUG_OVERLAY_CATEGORY_3 | SCREEN_DEBUG_OVERLAY_CATEGORY_4 | SCREEN_DEBUG_OVERLAY_CATEGORY_5),

	SCREEN_DEBUG_LAST = (1 << 13),
...
*
*   @param  disp The handle of the display whose display modes are being
*                   queried.
*   @param  max The maximum number of display modes that can be written to the
*               array of modes pointed to by @c param.
*   @param  modes The buffer where the retrieved display modes is stored.
*
*   @return @c 0 if successful, or @c -1 if an error occurred
*           (@c errno is set; refer to @c errno.h for more details).
*/
int screen_get_display_modes(screen_display_t disp, int max, screen_display_mode_t *modes);
...
*
*   @param  disp An instance of the display on which to perform the vsync operation.
*
*   @return @c 0 if successful, or @c -1 if an error occurred
*           (@c errno is set; refer to @c errno.h for more details).
*/
int screen_wait_vsync(screen_display_t disp);
...
*   @param  ctx The handle of the context in which the virtual display is to be
*               created. This context must have been created with the context type
...
*           (@c errno is set; refer to @c errno.h for more details).
*/
int screen_create_display(screen_display_t *pdisp, screen_context_t ctx, screen_display_mode_t *mode);
...
*           (@c errno is set; refer to @c errno.h for more details).
*/
int screen_read_window(screen_window_t win, screen_buffer_t buf, int count, const int *read_rects, int flags);
...
*   <b>Function Type:</b>  <a href="manual/rscreen_immediate_execution.xml">Immediate Execution</a>
*
*   This call blocks until there is a post event for the window you are waiting on, unless
*   @c SCREEN_DONT_BLOCK is specified.
*   This function is typically used in conjunction with @c screen_share_display_buffers()
*   and/or @c screen_share_window_buffers(). When using either function, Screen sends a post event to the
*   window you are waiting on (the consumer) after a buffer is posted to share or display by the producer.
*   The @c SCREEN_PROPERTY_FRONT_BUFFER property contains the posted buffer, even if this buffer 
*   hasn't been displayed yet.
*
*   When @c SCREEN_DONT_BLOCK is specified, the function returns immediately instead of waiting for the
*   next post to the window. In this case, @c SCREEN_PROPERTY_FRONT_BUFFER contains the most recently
*   posted buffer. If no buffers have been posted to share or display, @c SCREEN_PROPERTY_FRONT_BUFFER is NULL.
*
*
