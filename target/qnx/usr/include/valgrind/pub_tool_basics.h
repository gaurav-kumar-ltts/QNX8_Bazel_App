...
// For varargs types
#include <stdarg.h>
// Make sure that offsetof does not get redefined
#if defined(VGO_nto)
#include <stddef.h>
#endif
...
typedef
   struct {
      UWord _val;
#if defined(VGP_arm_nto)
      UWord _ext; /* if 64bit return is needed */
#endif
      Bool  _isError;
   }
   SysRes;
...
static inline UWord sr_Res ( SysRes sr ) {
   return sr._isError ? (UWord)-1 : sr._val;
}
...
static inline Bool sr_EQ ( UInt sysno, SysRes sr1, SysRes sr2 ) {
   return (sr1._val == sr2._val) && (!sr1._isError == !sr2._isError);
}
