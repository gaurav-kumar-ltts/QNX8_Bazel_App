...
#ifdef __QNXNTO__
/** @brief Unique descriptor for the BUS_CHILD_BIND() method */
extern struct kobjop_desc bus_child_bind_desc;
/** @brief A function implementing the BUS_CHILD_BIND() method */
typedef int bus_child_bind_t(device_t _dev, device_t _child);
/**
 * @brief Bind a device to a bus
 *
 * This method should return 0 if successful. It should return error if device
 * fails to bind to the bus.
 *
 * @param _dev		the parent device to bind on
 * @param _child	the device which is being bound
 */

static __inline int BUS_CHILD_BIND(device_t _dev, device_t _child)
{
	kobjop_t _m;
	int rc;
	KOBJOPLOOKUP(((kobj_t)_dev)->ops,bus_child_bind);
	rc = ((bus_child_bind_t *) _m)(_dev, _child);
	return (rc);
}
