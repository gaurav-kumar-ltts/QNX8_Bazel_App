...
#ifdef __QNXNTO__
/** @brief Unique descriptor for the MIIBUS_READEXTREG() method */
extern struct kobjop_desc miibus_readextreg_desc;
/** @brief A function implementing the MIIBUS_READEXTREG() method */
typedef int miibus_readextreg_t(device_t dev, int phy, int devad, int reg);

static __inline int MIIBUS_READEXTREG(device_t dev, int phy, int devad, int reg)
{
	kobjop_t _m;
	int rc;
	KOBJOPLOOKUP(((kobj_t)dev)->ops,miibus_readextreg);
	rc = ((miibus_readextreg_t *) _m)(dev, phy, devad, reg);
	return (rc);
}

/** @brief Unique descriptor for the MIIBUS_WRITEEXTREG() method */
extern struct kobjop_desc miibus_writeextreg_desc;
/** @brief A function implementing the MIIBUS_WRITEEXTREG() method */
typedef int miibus_writeextreg_t(device_t dev, int phy, int devad, int reg,
                                 int val);

static __inline int MIIBUS_WRITEEXTREG(device_t dev, int phy, int devad,
                                       int reg, int val)
{
	kobjop_t _m;
	int rc;
	KOBJOPLOOKUP(((kobj_t)dev)->ops,miibus_writeextreg);
	rc = ((miibus_writeextreg_t *) _m)(dev, phy, devad, reg, val);
	return (rc);
}
