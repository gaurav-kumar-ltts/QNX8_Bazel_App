...
#if defined(__EXT_QNX)
extern int clock_gettime_ns(clockid_t __clock_id, _Uint64t *__now_ns);
extern _Uint64t clock_gettime_rt_ns(void);
extern _Uint64t clock_gettime_mon_ns(void);

/**
 * @brief  Convert a clock cycle count to a time in nanoseconds.
 *
 * @param[in]  cycles  Clock cycle count.
 *
 * @return  Time in nanoseconds, or UINT64_MAX if the value is too large to be
 *          represented as a uint64_t.
 */
extern _Uint64t clock_cycles_to_ns(_Uint64t __cycles);

/**
 * @brief  Convert a clock cycle count to a time in nanoseconds relative to the
 *         specified system clock.
 *
 * @param[in]   cycles    Clock cycle count.
 * @param[in]   clock_id  System clock ID.
 * @param[out]  time_ns   System clock time in nanoseconds.
 *
 * @return  0 on success, otherwise -1 and errno is set to one of the following:
 *          @c EINVAL:  System clock ID isn't CLOCK_REALTIME, CLOCK_SOFTTIME or
 *                      CLOCK_MONOTONIC.
 *          @c ERANGE:  The resulting system clock time in nanoseconds can't be
 *                      represented via a uint64_t.
 */
extern int clock_cycles_to_time_ns(_Uint64t __cycles, clockid_t __clock_id,
        _Uint64t *__time_ns);
#endif
#if defined(__NYI)
...
extern int timer_timeout(clockid_t __id, unsigned __flags, const struct sigevent *__notify, const struct timespec *__ntime, struct timespec *__otime);
extern int timer_timeout_r(clockid_t __id, unsigned __flags, const struct sigevent *__notify, const struct timespec *__ntime, struct timespec *__otime);
...
static inline _Uint64t __timespec2nsec(const struct timespec *const __ts)
{
	...
	if (__builtin_mul_overflow((_Uint64t)__ts->tv_sec, 1000000000ULL, &result)) {
		return ~(_Uint64t)0;
	}

	if (__builtin_add_overflow(result, __ts->tv_nsec, &result)) {
		if (__ts->tv_nsec < 0) {
			// Underflow
			return 0;
		} else {
...
extern void timespecadd(__const struct timespec *a, __const struct timespec *b, struct timespec *result);
#endif
