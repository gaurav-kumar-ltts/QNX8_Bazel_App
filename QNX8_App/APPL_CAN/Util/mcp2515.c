...
#define MCP_READ_RXB0       0x90  // Read RX Buffer 0 Instruction
...
int mcp2515_hw_init(int fd) {
    ...
    mcp2515_write_register(fd, 0x2A, 0x00); // CNF1 (500kbps @ 8MHz setup)
    mcp2515_write_register(fd, 0x29, 0x91); // CNF2
    mcp2515_write_register(fd, 0x28, 0x01); // CNF3

    mcp2515_write_register(fd, MCP_CANCTRL, 0x00); // Normal Mode
    usleep(10000);
...
int io_devctl(resmgr_context_t *ctp, io_devctl_t *msg, RESMGR_OCB_T *ocb) {
    ...
    // Safely retrieve the attribute structure via container_of
